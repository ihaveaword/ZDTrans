...
💡 **提示**: 主题设置会实时同步到所有窗口（主窗口、弹出窗口、设置窗口）。

详见 [夜色模式使用指南](docs/NIGHT_MODE_GUIDE.md)
...
- [🌙 夜色模式](docs/NIGHT_MODE_GUIDE.md) - 主题切换指南
- [🎓 自定义学科领域](docs/CUSTOM_DOMAIN_GUIDE.md) - 添加专属领域
- [🏷️ 关键词配置](docs/KEYWORDS_GUIDE.md) - 提高翻译准确度
...
除了9个预设学科领域外，您还可以添加自定义领域：

**方法 1：通过设置界面**
1. 打开设置 → 翻译配置
2. 在"学科领域"下拉框中直接输入（例如：`量子计算`）
3. 点击右侧 `+` 按钮添加到列表
4. 保存设置
...
```json
{
  "translation": {
    "domain": "量子计算",
    "custom_domains": ["量子计算", "区块链技术", "人工智能伦理"]
  }
}
```
...
详见 [自定义学科领域指南](docs/CUSTOM_DOMAIN_GUIDE.md)
